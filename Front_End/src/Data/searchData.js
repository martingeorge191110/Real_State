...
const countriesList = (inputVal) => {
  const countries = ['Cairo', 'cairon', 'London', 'Cmona', 'Luxur', 'Lambur', 'Camern', 'Cabeno']
  let retArr = []

  for (let i = 0; i < countries.length; i++) {
    if (countries[i].toLowerCase().match(inputVal.toLowerCase()))
        retArr.push(countries[i])
  }
